...
def simulatedFrontier(item: simulatedFrontierItem):
    ...
    def get_annual_portfolio_performance(weights, mean_returns, cov_matrix):
        ...
        stdev = np.sqrt(np.dot(weights.T, np.dot(cov_matrix, weights))) * np.sqrt(252)
    ...
    results_DF.columns = ['std_dev','return','sharpe_ratio']
...
def EfficientFrontier(item: efficientFrontierItem):
    ...
    target = np.linspace(min_return, max(mean_returns)* 252, 50)
