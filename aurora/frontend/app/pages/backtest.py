...
def display_backtest():
    ...
    with st.beta_expander(label="Portfolio inputs"):
        ...
        start_date = date_col1.date_input("Start Date", value= datetime(2018, 12, 31)).strftime("%Y-%m-%d")
        end_date = date_col2.date_input("End Date",value= datetime(2020, 3, 31)).strftime("%Y-%m-%d")
        ...
        if rebalance:
            rebalance_frequency =  date_col2.selectbox(label='Rebalance frequency', options= list(rebalance_options.keys()))
            frequency = rebalance_options[rebalance_frequency]
        else:
            frequency = None
        

        selected_funds = st.multiselect(label = 'Fund selection', options = list(fund_list['Company']))
        portfolio = []
        amount_list = []
        ...
        for i in range(0,len(selected_funds)):
            portfolio.append({
                "fund": fund_list[fund_list['Company']== selected_funds[i]]['Code'].reset_index(drop=True)[0],
    ...
    if len(portfolio):
        ...
        with st.beta_expander(label="Portfolio historical projection"):
            ...
            chartoutput = alt.Chart(backtest_portfolio).mark_line().encode(
                x='date',
                y='portfolio'
